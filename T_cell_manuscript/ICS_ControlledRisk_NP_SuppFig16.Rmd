...
## Plot the controlled risk curve for the fold-rise markers

```{r controlled VE HIPP, echo=FALSE, message=FALSE, warning=FALSE}
controlled_ve_mk_vec = c(mk_vec_fr)
controlled_ve_names_vec = c(names_vec_fr)
...
write.csv(df_np, paste0( './source_data/SuppFig16_FR_HIPP_',  mk, '.csv'))

mk_dist = dt_PBMC_PP_HIPP %>%
  filter(ph2.m1.ics == 1) %>%
  select(!!sym(mk), wt.m1.ics)

p = plot_cr_fr(ests_np, mk_dist, overall_incidence,
                     xlab_name = controlled_ve_names_vec[i],
                     title_name = 'Marker-Specific Risk',
                     t0 = 165, x_range = c(-1, 2),
                    x_breaks = c(-1, -1, 0, 1, 2))
...
saveRDS(p, paste0('./raw_data_for_figure/', controlled_ve_mk_vec[i], '_FR_HIPP.RData'))

ggsave(paste0('./Figure/', controlled_ve_mk_vec[i], '_FR_HIPP.pdf'),
