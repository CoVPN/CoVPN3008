...
# Function processing the return from the cccs package
# HR per 3-fold increase
process_cccs_output_sd <- function(md){
  
  conf.int.linear = confint(md$coxph.fit) * log10(3)
  conf.int.exp = exp(conf.int.linear)
  ...
  tbl_output$estimate = exp(md$coxph.fit$coefficients * log10(3))
  tbl_output$conf.low = conf.int.exp[,'2.5 %']
  tbl_output$conf.high = conf.int.exp[,'97.5 %']
...
process_cox_output <- function(md2){
  tbl_output = broom::tidy(md2, exp = TRUE, conf.int = TRUE)
  #CI = as.data.frame(exp(confint(md2)))
